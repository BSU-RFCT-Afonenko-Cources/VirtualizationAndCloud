...
for d in bin etc proc sys dev home/ubuntu/rootfs-lab/tmp run var/www home/ubuntu/rootfs-lab/config home/ubuntu/rootfs-lab/data; do test -d "$B/$d" || { echo "Нет каталога $B/$d"; exit 1; }; done
test -x "$B/bin/busybox" || { echo "Нет исполняемого BusyBox"; exit 1; }
test -L "$B/bin/sh" || { echo "Нет applet-ссылки /bin/sh"; exit 1; }
test "$(stat -c %a "$B/home/ubuntu/rootfs-lab/tmp")" = "1777" || { echo "Неверные права /home/ubuntu/rootfs-lab/tmp"; exit 1; }
