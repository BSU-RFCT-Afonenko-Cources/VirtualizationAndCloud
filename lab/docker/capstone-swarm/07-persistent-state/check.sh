...
jq -e '.[] | select(.Type=="volume" and .Source=="market_db-data" and .Target=="/home/ubuntu/capstone-swarm/postgresql-data")' <<<"$mounts" >/dev/null || fail "DB volume mount неверен"
...
out=/home/ubuntu/capstone-swarm/capstone-persistence.json
rm -f "$out"
