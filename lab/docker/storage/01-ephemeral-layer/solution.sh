...
/usr/bin/docker run -d --name storage-ephemeral alpine:3.20 /bin/sh -c 'mkdir -p /home/ubuntu/storage && printf "%s\n" "writable-layer-data" > /home/ubuntu/storage/ephemeral-marker; while :; do sleep 3600; done' >/dev/null
