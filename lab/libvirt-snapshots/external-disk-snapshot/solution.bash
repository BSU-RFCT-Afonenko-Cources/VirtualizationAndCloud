...
if ! /usr/bin/virsh -c qemu:///system snapshot-info lab-vm lab-ext-001 >/dev/null 2>&1; then
  ...
  /usr/bin/test "$ACTIVE" != /home/ubuntu/external-disk-snapshot/libvirt-snapshots/lab-ext-001.qcow2
  /usr/bin/rm -f /home/ubuntu/external-disk-snapshot/libvirt-snapshots/lab-ext-001.qcow2
  /usr/bin/virsh -c qemu:///system snapshot-create-as lab-vm lab-ext-001 --description 'Crash-consistent external disk snapshot' --disk-only --atomic --diskspec vda,snapshot=external,file=/home/ubuntu/external-disk-snapshot/libvirt-snapshots/lab-ext-001.qcow2 >/dev/null
fi
