...
if /usr/bin/virsh -c qemu:///system snapshot-info lab-vm lab-quiesce-001 >/dev/null 2>&1; then
  ...
  if /usr/bin/test "$ACTIVE" != /home/ubuntu/quiesced-snapshot/libvirt-snapshots/lab-quiesce-001.qcow2; then /usr/bin/rm -f /home/ubuntu/quiesced-snapshot/libvirt-snapshots/lab-quiesce-001.qcow2; fi
  if /usr/bin/virsh -c qemu:///system snapshot-create-as lab-vm lab-quiesce-001 --description 'Quiesced external snapshot' --disk-only --quiesce --atomic --diskspec vda,snapshot=external,file=/home/ubuntu/quiesced-snapshot/libvirt-snapshots/lab-quiesce-001.qcow2 >"$DIR/quiesce-success.txt" 2>"$DIR/quiesce-error.txt"; then
