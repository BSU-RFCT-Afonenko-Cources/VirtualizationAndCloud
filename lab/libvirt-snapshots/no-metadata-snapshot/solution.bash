...
if /usr/bin/test "$ACTIVE" = /home/ubuntu/no-metadata-snapshot/libvirt-snapshots/lab-nometa-001.qcow2; then
  /usr/bin/printf 'lab-nometa-001 overlay already active\n' >"$DIR/no-metadata-result.txt"
else
  /usr/bin/rm -f /home/ubuntu/no-metadata-snapshot/libvirt-snapshots/lab-nometa-001.qcow2
  /usr/bin/virsh -c qemu:///system snapshot-create-as lab-vm lab-nometa-001 --description 'No-metadata live backup overlay' --disk-only --atomic --no-metadata --diskspec vda,snapshot=external,file=/home/ubuntu/no-metadata-snapshot/libvirt-snapshots/lab-nometa-001.qcow2 >"$DIR/no-metadata-result.txt"
fi
