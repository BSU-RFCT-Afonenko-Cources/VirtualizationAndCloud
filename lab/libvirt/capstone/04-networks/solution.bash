...
/usr/bin/cat >$D/cap-front.xml <<'XML'
...
XML
/usr/bin/cat >$D/cap-back.xml <<'XML'
...
XML
for n in cap-front cap-back; do "${VIRSH[@]}" net-info "$n" >/dev/null 2>&1 || "${VIRSH[@]}" net-define "$D/$n.xml" >/dev/null; "${VIRSH[@]}" net-start "$n" >/dev/null 2>&1 || true; "${VIRSH[@]}" net-autostart "$n" >/dev/null; "${VIRSH[@]}" net-dumpxml "$n" >"$D/$n.xml"; done
